...
export function buildDonorImpactTrail(
  ...
  const humanizeProgrammeId = (id: string) => {
    if (!id) return '';
    if (/^[0-9a-f-]{36}$/i.test(id)) return `Programme (${id.slice(0, 8)}…)`;
    return id
      .replace(/-/g, ' ')
  ...
  const programmes = aggregateAll
    ...
    .map(a => ({
      ...
      programLabel:
        [...programLabelsTouched].find((l) => programIdFromName(l) === a.programId)
        ?? a.programId.replace(/-/g, ' '),
