...
const WhatsAppPortal: React.FC = () => {
  ...
  const [fieldOrgCode, setFieldOrgCode]   = useState<string | null>(null);
  const [fieldOrgHint, setFieldOrgHint]   = useState<string | null>(null);

  /* eslint-disable react-hooks/set-state-in-effect -- hydrate local form from saved config */
  useEffect(() => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await apiFetch('/integrations/whatsapp/code');
        if (!res.ok) return;
        const data = await res.json();
        if (data?.org_code) setFieldOrgCode(String(data.org_code));
        if (data?.instructions) setFieldOrgHint(String(data.instructions));
      } catch {
        /* ignore */
      }
    };
  ...
  return (
    <div>
      {fieldOrgCode && (
        ...
        >
          <div style={{ fontWeight: 700, color: '#0f766e', marginBottom: '0.35rem' }}>Field onboarding code</div>
          <code style={{ fontSize: '1.05rem', letterSpacing: '0.04em' }}>{fieldOrgCode}</code>
          {fieldOrgHint && (
            <p style={{ margin: '0.5rem 0 0', color: '#134e4a', lineHeight: 1.45 }}>{fieldOrgHint}</p>
          )}
        </div>
      )}
