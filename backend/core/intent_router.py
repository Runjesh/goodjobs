...
def route_intent(user_input: str, ngo_id: Optional[str] = None) -> Dict[str, Any]:
    prompt = ChatPromptTemplate.from_template(INTENT_PROMPT)
    llm = chat_openai(ngo_id=ngo_id, model="gpt-4o-mini", temperature=0)
    if llm is None:
        return _fallback_card(user_input)
    ...
    try:
        ...
        return _fallback_card(user_input, error=str(e))


def _fallback_card(user_input: str, error: str = "") -> Dict[str, Any]:
    return {
        "intent_type": "unknown",
        "summary": (
            f"I heard '{user_input}', but no LLM key is configured or the model call failed. "
            "Add an OpenAI API key under Settings → AI & Agents (or set OPENAI_API_KEY on the server)."
