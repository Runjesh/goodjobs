...
-- ═══════════════════════════════════════════════════════════════════════════
-- Platform scale — audit log, WA queue, field intake, TEXT RLS helpers
-- ═══════════════════════════════════════════════════════════════════════════
...
CREATE INDEX IF NOT EXISTS idx_platform_audit_ngo_created ON platform_audit_log (ngo_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_wa_field_intake_ngo ON wa_field_intake (ngo_id, created_at DESC);

CREATE OR REPLACE FUNCTION current_ngo_id_text() RETURNS TEXT AS $$
    SELECT NULLIF(current_setting('app.current_ngo_id', true), '');
$$ LANGUAGE sql STABLE;

ALTER TABLE program_beneficiaries ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ngo_isolate_program_beneficiaries ON program_beneficiaries;
CREATE POLICY ngo_isolate_program_beneficiaries
...
DROP POLICY IF EXISTS ngo_isolate_intent_queue ON intent_queue;
CREATE POLICY ngo_isolate_intent_queue
    ON intent_queue FOR ALL
    USING (ngo_id = current_ngo_id_text())
    WITH CHECK (ngo_id = current_ngo_id_text());
